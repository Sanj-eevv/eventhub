...
<template>
    <header
        class="fixed top-0 left-0 right-0 z-50 bg-sf-bg/90 backdrop-blur-md border-b border-sf-border-subtle transition-colors duration-200"
    >
        <div class="mx-auto max-w-7xl px-5 sm:px-8">
            <div class="flex h-16 items-center justify-between">
                <Link :href="home()" class="flex items-center gap-2.5 group">
                    <span
                        class="inline-block w-1.5 h-1.5 rounded-full bg-sf-gold group-hover:scale-150 transition-transform duration-300"
                    />
                    <span
                        class="font-display text-base font-semibold tracking-[0.25em] uppercase text-sf-text"
                    >
                        {{ appName }}
                    </span>
                </Link>
        ...
        >
            ...
            <template v-if="user">
                ...
                <Link
                    ...
                    method="post"
                    as="button"
                    class="block w-full text-left py-3 text-sm text-sf-muted hover:text-sf-text transition-colors"
                    @click="handleLogout"
                    >Sign out</Link
                >
            </template>
            <template v-else>
                ...
                <Link
                    :href="registerCreate()"
                    class="block py-3 text-sm text-sf-ember hover:text-sf-ember-hover transition-colors"
                    >Register</Link
                >
            </template>
        </div>
