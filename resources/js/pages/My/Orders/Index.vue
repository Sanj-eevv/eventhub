    ...
    <HomeLayout>
        ...
        <PageContainer>
            ...
            <div v-if="orders.data.length === 0" class="py-24 text-center">
                ...
                >
                    <svg
                        class="h-6 w-6 text-sf-tertiary"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        stroke-width="1.5"
                ...
                <p class="font-body text-sm text-sf-tertiary mt-1 mb-6">
                    Your ticket purchases will appear here.
                </p>
                <Link
                    href="/events"
                    class="inline-flex items-center gap-2 px-5 py-2.5 bg-sf-ember text-white font-body text-sm rounded hover:bg-sf-ember-hover transition-colors"
                >
                    Browse Events
                </Link>
            </div>
                ...
                >
                    <div
                        :class="[
                            ...
                            statusDotClass[order.status.value] ??
                                'bg-sf-tertiary',
                        ]"
                    />
                    <div class="flex-1 min-w-0">
                        <p
                            class="font-display text-base font-medium text-sf-text truncate group-hover:text-sf-text transition-colors"
                        >
                            {{ order.event!.title }}
                        </p>
                        <div class="flex items-center gap-3 mt-1">
                            <span
                                v-if="order.paid_at"
                                class="font-body text-xs text-sf-tertiary shrink-0"
                                >Paid {{ formatDateTime(order.paid_at) }}</span
                            >
                        </div>
                    </div>
                    <div class="flex items-center gap-4 shrink-0">
                        <div class="text-right">
                            ...
                            <span
                                :class="[
                                    'inline-block font-body text-xs px-2 py-0.5 rounded border mt-1',
                                    statusBadgeClass[order.status.value] ??
                        ...
                        <svg
                            class="h-4 w-4 text-sf-tertiary group-hover:text-sf-muted group-hover:translate-x-0.5 transition-all"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            stroke-width="2"
                ...
                <template #loading>
                    <div class="mt-3 space-y-3">
                        <div
                            v-for="i in 3"
                            :key="i"
                            class="h-18.5 rounded-xl bg-sf-surface-raised animate-pulse"
                        />
                    </div>
                </template>
            </InfiniteScroll>
